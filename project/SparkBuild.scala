...
object BuildCommons {
  ...
  val connectProjects@Seq(connectCommon, connect, connectClient, connectClientComp) = Seq(
    "connect-common", "connect", "connect-client-jvm", "connect-client-jvm-compatibility"
  ).map(ProjectRef(buildLocation, _))
...
object SparkBuild extends PomBuild {
  ...
  enable(SparkConnectClientCompatibility.settings)(connectClientComp)
...
object CopyDependencies {
  ...
  lazy val settings = Seq(
    copyDeps := {
      ...
      val fidClientComp = (LocalProject("connect-client-jvm-compatibility") / assembly).value
      ...
      (Compile / dependencyClasspath).value.map(_.data)
        ...
        .foreach { jar =>
          ...
          } else if (jar.getName.contains("connect-client-jvm-compatibility") &&
            !SbtPomKeys.profiles.value.contains("noshade-connect-client-jvm-compatibility")) {
            Files.copy(fidClientComp.toPath, destJar.toPath)
