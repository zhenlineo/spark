...
class UDFClassLoadingE2ESuite extends RemoteSparkSession {
  ...
  // This avoid the need to depend on any test classes, so that this udf dose not need the test jar
  // to be in the server session classpath.
  ...
  test("update class loader after stubbing: new session") {
    // Session1 should stub the Hello class, but fail to call methods on it
    ...
    // Session2 uses the real Hello class
  ...
  test("update class loader after stubbing: same session") {
    // Session should stub the Hello class, but fail to call methods on it
    ...
    // Session uses the real Hello class
